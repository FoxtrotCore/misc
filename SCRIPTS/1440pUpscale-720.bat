...
SET aeo=
SET afo=
SET /p rate="Enter Frame Rate of Video. (##): "
...
SET sc=
:setupEnglishSub
SET /p se="Are there English Subtitles in .ssa format. (y/n): "
ECHO(
...
SET sc=-c:s copy 
:setupFrenchSub
SET /p sf="Are there French Subtitles in .ssa format. (y/n): "
ECHO(
...
ECHO(
ffmpeg -i "%f%" -vf yadif -c:v png "%~dp0480Frames\OEP%%06d.png" || goto :P1Error
ECHO(
...
ECHO(
"%~dp0waifu2x-caffe\waifu2x-caffe-cui.exe" -i "%~dp0480Frames" -e png  -l png -m noise_scale -h 1440 -n 1 -p cudnn -c 512 -o "%~dp01440Frames" --auto_start 1 --auto_exit 1 --no_overwrite 1 -y upconv_7_photo || goto :P2Error
...
:noEnglishSub
call :ColorText 0c "Script Terminated, there is no English Input Subtitle Source in "Import", or it is not labeled SPE.ssa"
ECHO(
pause
goto :eof

:noFrenchSub
call :ColorText 0c "Script Terminated, there is no French Input Subtitle Source in "Import", or it is not labeled SPE.ssa"
ECHO(
pause
goto :eof
