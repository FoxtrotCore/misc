...
ECHO(
SET fname=Code Lyoko - s%s%e%e%.mp4
SET f=%~dp0Import\%fname%
SET o=%~dp0Export\Code Lyoko - s%s%e%e%.mkv
...
if not exist "%~dp0Import\EP.mp4" (goto :inputVideoGone)
...
ECHO(
ffmpeg -i "%f%" -c:v png "%~dp0480Frames\OEP%%06d.png" || goto :P1Error
ECHO(
...
ECHO(
"%~dp0waifu2x-caffe\waifu2x-caffe-cui.exe" -i "%~dp0480Frames" -e png  -l png -m noise_scale -h 1440 -n 1 -p cudnn -c 256 -o "%~dp01440Frames" --auto_start 1 --auto_exit 1 --no_overwrite 1 -y upconv_7_photo || goto :P2Error
...
ECHO(
ffmpeg -i "%~dp0Import\EP.mp4" -map 0:1 -vn -movflags +faststart -acodec aac -b:a 96k "%~dp0Temp\AudioEPF.aac"
ECHO(
ffmpeg -i "%f%" -map 0:1 -vn -movflags +faststart -acodec aac -b:a 96k "%~dp0Temp\AudioEPE.aac"
ECHO(
call :ColorText 0B "Extraction Complete"
ECHO(
